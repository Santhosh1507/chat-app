    ...
    return (
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            <div className='pb-5'>
                ...
                <div className="flex items-center gap-2 bg-[#282142] px-3 py-2 rounded-full mb-4">
                    <img src={assets.search_icon} alt="search" className="w-4 h-4 opacity-70" />
                    <input
                    onChange={(e)=>setInput(e.target.value)}
                        type="text"
                        placeholder="Search user..."
                        className="bg-transparent border-none outline-none text-sm text-white placeholder:text-gray-400 flex-1"
                    />
                </div>
            </div>
